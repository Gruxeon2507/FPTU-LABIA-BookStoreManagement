...
//    @Query(value = "select * from `User` u where u.userId not in (select us.userId from `User` us join User_Role ur on us.userId = ur.userId where ur.roleId = 1)",nativeQuery = true)
