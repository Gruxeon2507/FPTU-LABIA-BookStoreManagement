...
class RegisterUser extends Component {
  ...
  changeUsernameHandler = (event) => {
    ...
    if (this.state.username.trim() !== "") {
      fetch(`http://192.168.182.56:6789/api/users/check/${inputUsername}`)
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.usernameError === "Username already taken") {
      ...
      fetch("http://192.168.182.56:6789/api/users/register", {
        method: "POST",
        headers: {
      ...
      if (this.state.checkUsername) {
        ...
        if (confirm) {
          window.location.href = "http://192.168.182.56:3000/login";
        }
      }
    }
  };
